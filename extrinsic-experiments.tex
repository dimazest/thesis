...
This chapter studies the relationship between the lexical representations and various methods of composition and looks for optimal lexical representation for additive, multiplicative and Kronecker-based compositional methods.\footnotemark{}

\footnotetext{This chapter includes the results presented in \newcite{milajevs-EtAl:2014:EMNLP2014} at EMNLP 2014.}

Here we report the results on the compositional datasets KS14 \cite{kartsadrqpl2014}, GS11 \cite{Grefenstette:2011:ETV:2140490.2140497} and PhraseRel (Chapter~\ref{sec:phraserel}). The evaluation is done similarly to the lexical evaluation (Chapter~\ref{sec:lexical}). If a dataset provides several human judgements for a sentence pair, the judgements are averaged before computing the correlation.
...
This chapter investigates how well a single model can perform on all tasks in contrast to the task (or dataset) tailored models of previous chapters. This is achieved by performing the evaluation on a combined score over the previous two lexical and three phrasal datasets. We not only combine the datasets together, but later in the chapter look for parameters that are good across all the datasets with all the compositional operators. Once the optimal lexical parameters are identified, they are tested on categorical compositional methods.
