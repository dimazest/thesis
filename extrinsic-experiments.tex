...
For multiplication, there are three most beneficial choices: for $D < 10\,000$ $k = 0.5$ ($k=0.2$ and $k=0.7$ are statistically indistinguishable), for $10\,000 \leq D < 30\,000$ $k = 0.7$ ($k=0.2$, $k=0.5$ and $k=1$ are statistically indistinguishable) and, finally, for $D > 30\,000$ $k = 1$ ($k=0.5$ and $k=0.7$ are statistically indistinguishable).
...
\footnotetext{The perfect score of 1 does not mean that the model is perfect, it means that for all queries at least one relevant document was among the top three ranked.}
...
For addition, 1 is the best choice of \texttt{neg}, but the performance of $k$ values follows H\ref{hyp:neg}: the more dimensions a model has, the sparser it should be. However, the only statistically significantly different results are with $k=7$ with $D \le 3\,000$, and $k=5$ with $D = 1\,000$.
...
For the tested datasets, the observed differences are often not statistically significant. The minimum significant difference depends on the dataset size: the larger the dataset, the smaller the minimum difference. For KS14 with 108 items, the minimum significant difference is $\sigma^{0.9}_{0.05} = 0.07$. For GS11 with 200 items, the minimum significant difference is $\sigma^{0.9}_{0.05} = 0.05$. For PhraseRel---despite the fact that it consists of 644 query-document pairs---the minimum significant difference is huge $\sigma_{0.05} = 0.43$, because of a different evaluation metric. Such a high value makes the results statistically indistinguishable. Ideally, new datasets should be developed to evaluate compositional semantics models that make sure that the dataset size and the evaluation metric lead to a small minimum required difference.
...
Regarding the hypotheses, we clearly see on Figure~\ref{fig:universal-freq} that there is no statistically significant difference between 1 and $\log n$ frequencies for the low-dimensional spaces, but $\log n$ is the best choice for the high-dimensional spaces, which is consistent with H\ref{hyp:freq}.
...
The SVOPN model with the score of 0.61 statistically significantly outperforms our best (0.52) on GS11. While they get a higher score on KS14 of 0.74 (though it is obtained with the \newcite{hashimoto-EtAl:2014:EMNLP2014} baseline) it is still lower than all of our results, except copy-subject and copy-object, but the improvement is not statistically significant. Interestingly, our result of 0.74 with copy-subject is close to their score but is still lower.

\newcite{hashimoto-tsuruoka:2016:P16-1} jointly learn compositional and non-compositional phrase embeddings by using a compositionality scoring function. They improve on their previous work and get the score of 0.68 on GS11 versus our 0.52 with a statistically significant difference.
