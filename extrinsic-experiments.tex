...
This chapter investigates to how well a single model can perform on all tasks in contrast to the task (or dataset) tailored models. This is achieved by performing the evaluation on a combined score over two lexical and three phrasal datasets.
...
On the GS11 dataset, our spaces improve over the corresponding operators on all but two compositional methods (the exceptions being copy object and Frobenius mult). Only multiplication and Kronecker improve over the best reported score of 0.456 in \newcite{milajevs-EtAl:2014:EMNLP2014}. Kronecker yields the highest score of 0.516.

\newcite{kim2015neural} adopt the evaluation procedure of \newcite{milajevs-EtAl:2014:EMNLP2014} to test an extended word2vec model that is tuned for multiplicative interaction of the vectors, not additive. They improve on most of the composition operators on the KS14 and GS11 datasets.
...
The SVOPN model with the score of 0.614 outperforms our best (0.516) on GS11. While they get a higher score on KS14 of 0.744 (though it is obtained with the \newcite{hashimoto-EtAl:2014:EMNLP2014} baseline) it is still lower than all our results except copy-subject and copy-object. Interestingly, our result of 0.738 with copy-subject is close to their score, but is still lower.
