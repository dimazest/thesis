...
\chapter[Universal parameter selection]{Universal parameter selection for lexical and compositional datasets}
\label{sec:universal-param-selection}

\todo[inline]{From lexical to compositional.}

\section{Max selection}
...
Performance of the models selected manually shown on Table~\ref{tab:universal-heuristics-selection}. Again, there is a lot consistency between parameters. Global context probabilities are chosen most of the times. The linear model achieves $R^2 = 0.828$. The most influencing parameters are \texttt{neg}, \texttt{freq} and compositional operator, refer to Table~\ref{tab:universal-ablation}.

Multiplication gives the best results on lexical tasks, together with PhraseRel. The scores are 0.378, 0.758 and 0.893, respectively. For the dimensions of 20000, 30000 and 40000, heuristics for multiplication match the max-based selection. Kronecker is strong in compositional tasks, scoring 0.792 for KS14, 0.516 for GS11 and 0.893 for PhraseRel.

Refer to Figure~\ref{fig:universal-parameters} for the parameter average behaviour.
...
On lexical datasets, there is little difference between the model selection methods especially for spaces with more than 30000 dimensions, as Figure~\ref{fig:uniersal-results-simlex} and Figure~\ref{fig:universal-results-men} show. As Kronecker was tested only with the inner product as the similarity measure for spaces with dimensionality greater than 3000, Kronecker does not follow this patter of addition and multiplication.

For compositional tasks, dimensionality does not contribute as much as in case of lexical, the best results are achieved with low dimensional models ($D \leq 10000$, with an exception of GS11 and Kronecker, where a models with 50000 gives the highest result) Refer to Figures~\ref{fig:universal-results-ks14}, \ref{fig:universal-results-gs11} and \ref{fig:universal-results-phraserel}.

\todo[noline]{Probably this should be one of the major conclusions.}
However, when the models selected on lexical tasks are applied in a compositional setting, they perform worse than the models selected by the universal score. This suggests that a model that is good on lexical tasks will not necessarily perform well on a compositional task. In addition, sometimes this difference increases as dimensionality increases, for example this is the case for multiplication.
