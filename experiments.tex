...
Max selection neither picks the best possible results when transferred from MEN to SimLex-999, but the average difference is lower: it is 0.042, but in this case it is similar to the transition in other direction.
...
Local context probabilities perform better in compositional tasks. Multiplication benefits from the unsmoothed distribution probability, while highly dimensional models perform best with smoothing ($\alpha = 0.75$). The only exception is additive models with $D < 5000$, where global probabilities perform best.

For low dimensional spaces, addition performs best with dense spaces ($k > 1$, $D < 5000$), but for high dimensional spaces, addition performs best  with sparser spaces ($k = 0,7$, $D \geq 5000$).
